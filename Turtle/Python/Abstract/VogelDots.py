...
def Vogel(angle=2.0*pi/((1.0+sqrt(5.0))/2.0), k=2.0, origin=(0,0), points=5000):
    ...
    hue = angle*(random.randint(1, 5)) # convert angle to hue value
    r,g,b = colorsys.hsv_to_rgb(hue,1,1) # convert HSV to RGB
    turtle.pencolor(int(r*255),int(g*255),int(b*255)) # set pencolor to RGB values
    ...
    for i in range(1,points):
        ...
        hue = a/(2*pi) # convert angle to hue value
        r,g,b = colorsys.hsv_to_rgb(hue,1,1) # convert HSV to RGB
        turtle.pencolor(int(r*255),int(g*255),int(b*255)) # set pencolor to RGB values
